...
- Follow `docs/dev/ui-visual-design.md` for all UI work. The user-authorized `idtech5-ui` replacement reconstructs Quake 4 framing, controls and symbols as editable vector art, preserves the 45-degree motif, and permits bitmaps only for complex imagery. Retain stock references for fidelity; do not add extracted proprietary art to Git. Track full runtime/editor/migration completion in `docs/dev/plans/idtech5-ui.md`.
