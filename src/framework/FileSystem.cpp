...
void idFileSystemLocal::Startup( void ) {
	...
	if ( fs_validateOfficialPaks.GetBool() ) {
		// Inside the guard, not ahead of it: a port that repacks pak0/pak1
		// needs one escape from a fatal mismatch, and this cvar is CVAR_INIT
		// so the command line is the only place it can be set.
